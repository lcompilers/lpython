    ...
    for i in range(size1):
        for j in range(size2):
            for k in range(size3):
                assert abs( arctanh(array[i, j, k])**2 - result[i, j, k]) <= eps
...
def elemental_arctanh():
    ...
    for i in range(100):
        for j in range(50):
            for k in range(10):
                arraynd[i, j, k] = sin(((-1)**k) * float(i + j + k))

    arctanhnd =  arctanh(arraynd)**2
...
def elemental_trig_identity():
    ...
    identity1 = 2 * arctanh(arraynd) - arctanh((2 * arraynd) / ( 1 + arraynd**2))
    identity2 =  cosh(arctanh(arraynd)) - (sqrt(1 - (arraynd**2)))**(-1)
