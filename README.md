...
Create a Conda environment using the preexisting environment file (conda/base.yaml):

```bash
conda env create -f conda/base.yml
conda activate lp
```

Use `conda/base.yaml` if you want to use compilers from the operation system,
otherwise, use `conda/compilers.yaml`
