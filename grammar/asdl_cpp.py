...
class PickleVisitorVisitor(ASDLVisitor):
    ...
    def visitModule(self, mod):
        ...
        self.emit(  "int indent_level = 0, indent_spaces = 3, tmp = 0, tmp1 = 0, lvl = 0;", 1)
        ...
        self.emit(      "if(indent_level > 0) {", 2)
        self.emit(          "indent_level--;", 3)
        self.emit(          "LFORTRAN_ASSERT(indent_level >= 0);", 3)
        self.emit(          "indtd = std::string(indent_level*indent_spaces, ' ');",3)
        self.emit(      "}",2)
        self.emit(  "}",1)
        self.mod = mod
