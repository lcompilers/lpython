...
class DiffTemplateFunctionVisitor : public ASR::BaseVisitor<DiffTemplateFunctionVisitor>
{
public:
    Allocator &al;
    SymbolTable *current_scope;
    ...
    DiffTemplateFunctionVisitor(Allocator &al, std::map<std::string, ASR::ttype_t*> subs,
    ...
    void visit_TemplateFunction(const ASR::TemplateFunction_t &x) {
        ...
        for (size_t i=0; i<x.n_args; i++) {
            ASR::symbol_t *param_sym = ASR::down_cast<ASR::Var_t>(x.m_args[i])->m_v;
            this->visit_symbol(*param_sym);
            ASR::symbol_t *new_param_sym = ASR::down_cast<ASR::symbol_t>(tmp);
            std::string var_name = ASRUtils::symbol_name(new_param_sym);
            current_scope->add_symbol(var_name, new_param_sym);

            ASR::symbol_t *var = current_scope->get_symbol(var_name);
            args.push_back(al, ASRUtils::EXPR(ASR::make_Var_t(al, x.base.base.loc, var)));
        }

        ASR::symbol_t *return_sym = ASR::down_cast<ASR::Var_t>(x.m_return_var)->m_v;
        this->visit_symbol(*return_sym);
        ASR::symbol_t *new_return_sym = ASR::down_cast<ASR::symbol_t>(tmp);
        std::string return_var_name = ASRUtils::symbol_name(new_return_sym);
        current_scope->add_symbol(return_var_name, new_return_sym);
...
public:
    ...
    ASR::symbol_t* get_generic_function(std::map<std::string, ASR::ttype_t*> subs,
        ...
        DiffTemplateFunctionVisitor tf(al, subs, current_scope, new_function_num);
    ...
    // Given certain arguments, check if a generic function corresponding to
    // that signature has already been instantatiated or not.
    int find_generic_function(std::string func_name, Vec<ASR::expr_t*> args) {
        int generic_number;
        if (generic_defs.find(func_name) != generic_defs.end()) {
            std::map<int, Vec<ASR::expr_t*>> generic_map = generic_defs[func_name];
            for (auto &expr_map: generic_map) {
                Vec<ASR::expr_t*> exprs = expr_map.second;
                bool flag = true;
                for (size_t i=0; i<exprs.n; i++) {
                    ASR::expr_t *arg = args[i];
                    ASR::expr_t *param = exprs[i];
                    ASR::ttype_t *arg_type = ASRUtils::expr_type(arg);
                    ASR::ttype_t *param_type = ASRUtils::expr_type(param);
                    if (!ASRUtils::check_equal_type(arg_type, param_type)) {
                        flag = false;
                    }
                }
                if (flag) {
                    generic_number = expr_map.first;
                    return generic_number;
                }
            }
            generic_number = generic_map.size();
            generic_map[generic_number] = args;
        } else {
            generic_number = 0;
            std::map<int, Vec<ASR::expr_t*>> new_map;
