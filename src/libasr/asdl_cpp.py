...
class TreeVisitorVisitor(ASDLVisitor):
    ...
    def visitModule(self, mod):
        ...
        self.emit(  "std::vector<int> indent_length;", 1)
        self.emit("public:")
        self.emit(  "TreeBaseVisitor() : use_colors(false), last(true), attached(false) { s.reserve(100000); }", 1)
        self.emit(  "void inc_indent() {", 1)
        self.emit(      "indent_length.push_back(2);", 2)
        self.emit(      'indtd += "  ";', 2)
        self.emit(  "}", 1)
        self.emit(  "void inc_lindent() {", 1)
        self.emit(      "indent_length.push_back(4);", 2)
        self.emit(      'indtd += "│ ";', 2)
        self.emit(  "}", 1)
        self.emit(  "void dec_indent() {", 1)
        self.emit(      "LCOMPILERS_ASSERT(indent_length.size() > 0);", 2)
        self.emit(      "int prev_indent = indent_length.back();", 2)
        self.emit(      "indent_length.pop_back();", 2)
        self.emit(      "LCOMPILERS_ASSERT(prev_indent > 0);", 2)
        self.emit(      "indtd = indtd.substr(0, indtd.size() - prev_indent);",2)
        self.emit(  "}", 1)
        self.mod = mod
