...
uint64_t get_hash_c(ASR::asr_t *node)
{
...
class ASRToCVisitor : public ASR::BaseVisitor<ASRToCVisitor>
{
public:
    diag::Diagnostics &diag;
    std::map<uint64_t, SymbolInfoC> sym_info;
    std::string src;
    ...
    ASRToCVisitor(diag::Diagnostics &diag) : diag{diag} {}
...
#include <lfortran_intrinsics.h>
    ...
    void visit_Subroutine(const ASR::Subroutine_t &x) {
        ...
        for (auto &item : x.m_symtab->get_scope()) {
            if (ASR::is_a<ASR::Variable_t>(*item.second)) {
                ...
                if (v->m_intent == LFortran::ASRUtils::intent_local) {
                    ...
                    sym_info[get_hash_c((ASR::asr_t*)v)] = s;
                }
            }
        }
        ...
        for (auto &item : x.m_symtab->get_scope()) {
            if (ASR::is_a<ASR::Variable_t>(*item.second)) {
                ASR::Variable_t *v = ASR::down_cast<ASR::Variable_t>(item.second);
                if (v->m_intent == LFortran::ASRUtils::intent_local) {
                    if (sym_info[get_hash_c((ASR::asr_t*) v)].needs_declaration) {
                        std::string indent(indentation_level*indentation_spaces, ' ');
                        decl += indent;
                        decl += convert_variable_decl(*v) + ";\n";
                    }
    ...
    void visit_Function(const ASR::Function_t &x) {
        if (std::string(x.m_name) == "size" && intrinsic_module ) {
            ...
            sym_info[get_hash_c((ASR::asr_t*)&x)] = s;
            src = "";
            return;
        } else if ((
    ...
    void visit_FunctionCall(const ASR::FunctionCall_t &x) {
        ...
        if (sym_info[get_hash_c((ASR::asr_t*)fn)].intrinsic_function) {
            if (fn_name == "size") {
