...
static inline void verify_args(const ASR::IntrinsicScalarFunction_t& x, diag::Diagnostics& diagnostics) {
    ASRUtils::require_impl(x.n_args <= 4, "Call to list.index must have at most four arguments",
        x.base.base.loc, diagnostics);
    ASR::ttype_t *list_type = ASR::is_a<ASR::Const_t>(*ASRUtils::expr_type(x.m_args[0]))
...
static inline ASR::asr_t* create_ListIndex(Allocator& al, const Location& loc,
    ...
    ASR::ttype_t *type = ASR::is_a<ASR::Const_t>(*ASRUtils::expr_type(list_expr))
                             ? ASRUtils::type_get_past_const(ASRUtils::expr_type(list_expr))
