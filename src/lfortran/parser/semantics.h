...
void set_decl_t(decl_t &d,
    ...
    kind_item_t* m_kind, size_t n_kind,
...
static inline decl_t* DECL(Allocator &al, const Vec<decl_t> &x,
        char *type, Vec<kind_item_t> kind,
...
static inline decl_t* DECL2(Allocator &al, const Vec<decl_t> &x,
...
static inline decl_t* DECL3(Allocator &al, ast_t* n,
        const Vec<dimension_t> *d, expr_t *e)
{
...
static inline parameter_item_t make_parameter_item_t(Location &l,
...
/*
static inline reduce_t* REDUCE_TYPE(const ast_t *f)
{
...
char *fn_type2return_type(const Vec<ast_t*> &v) {
    for (size_t i=0; i < v.size(); i++) {
        if (v[i]->type == astType::fn_mod) {
            FnMod_t *t = (FnMod_t*)v[i];
            return t->m_s;
        }
    }
    return nullptr;
...
#define VAR_DECL3(attr, l) make_Declaration_t(p.m_a, l, \
        DECL2b(p.m_a, attr), 1)
#define VAR_DECL4(id, l) make_Declaration_t(p.m_a, l, \
        nullptr, 0)
#define PARAMETER_ITEM(name, value, l) make_parameter_item_t(l, \
        name2char(name), down_cast<expr_t>(value))
#define VAR_DECL5(items, l) make_ParameterStatement_t(p.m_a, l, \
        items.p, items.size())
#define VAR_DECL6(id, id_list, l) make_Declaration_t(p.m_a, l, \
