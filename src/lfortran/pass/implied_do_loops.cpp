...
public:
    ...
    void create_do_loop(ASR::ImpliedDoLoop_t* idoloop, ASR::Var_t* arr_var, ASR::expr_t* arr_idx=nullptr) {
        ...
        for( size_t i = 0; i < idoloop->n_values; i++ ) {
            ...
            ASR::stmt_t* doloop_stmt = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(al, arr_var->base.base.loc, array_ref, idoloop->m_values[i], nullptr, nullptr));
            ...
            if( arr_idx != nullptr ) {
                ...
                ASR::stmt_t* assign_stmt = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(al, arr_var->base.base.loc, arr_idx, increment, nullptr, nullptr));
        ...
        if( x.m_value->type == ASR::exprType::ConstantArray ) {
            ...
            if( arr_init->n_args == 1 && arr_init->m_args[0] != nullptr && 
                ...
                if( unit.m_global_scope->scope.find(std::string(idx_var_name)) == unit.m_global_scope->scope.end() ) {
                    ...
                    ASR::stmt_t* assign_stmt = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(al, arr_var->base.base.loc, idx_var, const_1, nullptr, nullptr));
                ...
                for( size_t k = 0; k < arr_init->n_args; k++ ) {
                    ...
                    if( curr_init->type == ASR::exprType::ImpliedDoLoop ) {
                        ...
                        ASR::expr_t* array_ref = LFortran::ASRUtils::EXPR(ASR::make_ArrayRef_t(al, arr_var->base.base.loc, arr_var->m_v,
                                                                            args.p, args.size(), 
                                                                            LFortran::ASRUtils::expr_type(LFortran::ASRUtils::EXPR((ASR::asr_t*)arr_var)), nullptr));
                        ASR::stmt_t* assign_stmt = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(al, arr_var->base.base.loc, array_ref, arr_init->m_args[k], nullptr, nullptr));
                        implied_do_loop_result.push_back(al, assign_stmt);
                        ...
                        assign_stmt = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(al, arr_var->base.base.loc, idx_var, increment, nullptr, nullptr));
                        implied_do_loop_result.push_back(al, assign_stmt);
                    }
                }
            }
            ...
            for( int i = n_dims - 1; i >= 0; i-- ) {
                ...
                if( doloop == nullptr ) {
                    ...
                    ASR::stmt_t* assign = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(al, x.base.base.loc, ref, x.m_value, nullptr, nullptr));
