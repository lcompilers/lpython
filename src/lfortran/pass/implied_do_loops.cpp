...
public:
    ...
    void create_implied_do_loop(ASR::ImpliedDoLoop_t* idoloop, ASR::Var_t* arr_var, int arr_idx_offset) {
        ...
        ASR::expr_t* idx_offset = nullptr;
        if( arr_idx_offset > 0 ) {
            idx_offset = EXPR(ASR::make_ConstantInteger_t(al, arr_var->base.base.loc, arr_idx_offset, _type));
        }
        ...
        for( size_t i = 0; i < idoloop->n_values; i++ ) {
            ...
            ASR::expr_t* idx = EXPR(ASR::make_BinOp_t(al, arr_var->base.base.loc, 
                                                        ...
                                                        _type));
            if( arr_idx_offset != 0 ) {
                idx = EXPR(ASR::make_BinOp_t(al, arr_var->base.base.loc, 
                                            idx, ASR::binopType::Add, idx_offset, 
                                            _type));
            }
            ai.m_right = idx;
            ai.m_step = nullptr;
    ...
    void visit_Assignment(const ASR::Assignment_t &x) {
        if( x.m_value->type == ASR::exprType::ArrayInitializer ) {
            ...
            if( arr_init->n_args == 1 && arr_init->m_args[0] != nullptr && 
                ...
                create_implied_do_loop(idoloop, arr_var, 0);
