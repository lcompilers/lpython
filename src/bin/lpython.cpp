...
using LCompilers::parse_python_file;
...
{
    char *env_p = std::getenv("LCOMPILERS_KOKKOS_DIR");
    if (env_p) return env_p;
    std::cerr << "The code C++ generated by the C++ LFortran backend uses the Kokkos library" << std::endl;
    std::cerr << "(https://github.com/kokkos/kokkos). Please define the LCOMPILERS_KOKKOS_DIR" << std::endl;
    std::cerr << "environment variable to point to the Kokkos installation." << std::endl;
    throw LCompilers::LFortranException("LCOMPILERS_KOKKOS_DIR is not defined");
}
...
int emit_tokens(const std::string &infile, bool line_numbers, const CompilerOptions &compiler_options)
{
    ...
    std::vector<int> toks;
    std::vector<LCompilers::YYSTYPE> stypes;
    std::vector<LCompilers::Location> locations;
    LCompilers::diag::Diagnostics diagnostics;
    auto res = LCompilers::tokens(al, input, diagnostics, &stypes, &locations);
    ...
    for (size_t i=0; i < toks.size(); i++) {
        std::cout << LCompilers::pickle_token(toks[i], stypes[i]);
        if (line_numbers) {
    ...
    bool with_intrinsic_modules, CompilerOptions &compiler_options)
{
    ...
    LCompilers::Result<LCompilers::ASR::TranslationUnit_t*>
        r = LCompilers::LPython::python_ast_to_asr(al, *ast, diagnostics, true,
    ...
    if (compiler_options.tree) {
        std::cout << LCompilers::pickle_tree(*asr, compiler_options.use_colors,
    ...
    CompilerOptions &compiler_options)
{
    ...
    diagnostics.diagnostics.clear();
    LCompilers::Result<LCompilers::ASR::TranslationUnit_t*>
        r1 = LCompilers::LPython::python_ast_to_asr(al, *ast, diagnostics, true,
            compiler_options.symtab_only);
    std::cerr << diagnostics.render(input, lm, compiler_options);
    if (!r1.ok) {
        LFORTRAN_ASSERT(diagnostics.has_error())
    ...
    */
    ...
    if (backend == Backend::llvm) {
        if (t == "x86_64-pc-windows-msvc") {
            ...
            std::string CC = "cc";
            char *env_CC = std::getenv("LCOMPILERS_CC");
            if (env_CC) CC = env_CC;
...
//
//     LCompilers::CPreprocessor cpp(compiler_options);
//     LCompilers::LocationManager lm;
//     lm.in_filename = infile;
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        */
        ...
        //         asr = LCompilers::mod_to_asr(al, arg_mod_file);
        ...
        if (arg_c) {
            if (backend == Backend::llvm) {
                ...
                throw LCompilers::LFortranException("Unsupported backend.");
            }
        }
