...
""", False)) == "Subroutine(name='f', args=[], decl=[Declaration(vars=[decl(sym='a', sym_type='integer'), decl(sym='b', sym_type='integer')])], body=[Assignment(target='b', value=BinOp(left=BinOp(left=BinOp(left=Num(n='1'), op=Add(), right=Num(n='2')), op=Add(), right=Name(id='a')), op=Mul(), right=Num(n='3')))], contains=[])"
    assert dump(parse("""\
subroutine f(x, y)
integer, intent(out) :: x, y
x = 1
end subroutine
""", False)) == "Subroutine(name='f', args=[arg(arg='x'), arg(arg='y')], decl=[Declaration(vars=[decl(sym='x', sym_type='integer'), decl(sym='y', sym_type='integer')])], body=[Assignment(target='x', value=Num(n='1'))], contains=[])"
